...
class APB_seq_item(uvm_sequence_item, Randomized):
    ...
    def __init__(self, name):
        ...
        def presetn_dist(PRESETn):
            return 9 if PRESETn == 1 else 2

        def penable_dist(PENABLE):
            return 9 if PENABLE == 1 else 2

        def pwrite_dist(PWRITE):
            return 9 if PWRITE == 1 else 2

        self.add_constraint(presetn_dist)
        self.add_constraint(lambda PADDR: PADDR % 4 == 0)
        self.add_constraint(penable_dist)
        self.add_constraint(pwrite_dist)
